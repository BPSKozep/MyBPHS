  ...
  return (
    ...
    >
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex max-w-[60%] flex-col gap-1">
          ...
          <div className="flex items-center gap-1 text-xs text-gray-200">
            {/** biome-ignore lint/a11y/noSvgWithoutTitle: todo what is this */}
            <svg
              className="h-3 w-3"
              fill="none"
              stroke="currentColor"
      ...
      <div className="mb-3 grid grid-cols-3 gap-3 text-xs">
        ...
        <div className="flex flex-col items-center">
          <p className="mb-1 font-medium text-gray-400">Figyelmeztetések</p>
          <Badge
      ...
      {deployment.StepName && (
        <div className="mb-3 rounded-lg bg-[#565e85]/20 p-2">
          <p className="text-xs font-medium text-gray-400">Jelenlegi lépés</p>
          <p
        ...
        <div className="mt-2 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <p className="font-mono text-white">{deployment.ID}</p>
            </div>
            <div>
          ...
          <div className="space-y-1 text-xs">
            ...
            <div>
              <span className="text-gray-400">Utolsó frissítés:</span>
              <span className="ml-2 text-white">
          ...
          {(deployment.VMHost ?? deployment.VMName ?? deployment.DartIP) && (
            <div className="border-t border-gray-600 pt-2">
              <p className="mb-1 text-gray-400">VM/Hálózat részletek</p>
              <div className="space-y-1">
                {deployment.VMHost && (
                  <div>
                    <span className="text-gray-400">VM Host:</span>
                    <span className="ml-2 text-white">{deployment.VMHost}</span>
                  </div>
                )}
                {deployment.VMName && (
                  <div>
                    <span className="text-gray-400">VM gép neve:</span>
                    <span className="ml-2 text-white">{deployment.VMName}</span>
                  </div>
                )}
                {deployment.DartIP && (
                  <div>
                    <span className="text-gray-400">DART IP:</span>
                    <span className="ml-2 text-white">
                      {deployment.DartIP}:{deployment.DartPort}
                    </span>
                  </div>
                )}
              </div>
...
export default function LaptopDeploymentsClient() {
  ...
  useEffect(() => {
    ...
    previousDeploymentsRef.current = [...deployments];
  }, [deployments, playErrorSound, playSuccessSound]);

  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <Loading />
        <p className="mt-4 text-sm text-gray-300">Telepítések betöltése...</p>
      </div>
    );
  }
