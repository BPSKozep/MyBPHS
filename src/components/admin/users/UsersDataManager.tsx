...
export default function UsersDataManager() {
  ...
  const createADUserMutation = api.ad.createUser.useMutation({
    ...
    onError: (error) => {
      setAlertDialog({
        ...
        description: `Hiba történt az AD fiók létrehozásakor: ${error.message}`,
        type: "error",
      });
    },
  });
  ...
  const deleteADUsersMutation = api.ad.deleteUsers.useMutation({
    ...
    onError: (error) => {
      ...
      setAlertDialog({
        ...
        description: `Hiba történt az AD felhasználók törlésekor: ${error.message}`,
        type: "error",
      });
    },
  });
        ...
        deleteUsersMutation.mutate(Array.from(selectedRows), {
          onSuccess: () => {
            ...
            if (usersWithAD.length > 0) {
              const emailsToDelete = usersWithAD.map((user) => user.email);
  ...
  const isAllSelected =
    paginatedUsers.length > 0 && selectedRows.size === paginatedUsers.length;
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-2">
            ...
            <div className="flex w-full gap-2 sm:hidden">
              ...
              <Button
                ...
                disabled={
                  selectedRows.size === 0 || deleteUsersMutation.isPending
                }
            ...
            <div className="hidden items-center gap-2 sm:flex">
                ...
                <DialogContent className="max-w-2xl border-gray-600 bg-[#2e2e2e]">
                  <DialogHeader>
                    <DialogTitle className="text-white">
                      AD felhasználók az alkalmazáson kívül
                    </DialogTitle>
                    <DialogDescription className="text-gray-300">
                      Ezek a felhasználók léteznek az Active Directory-ban, de
                      nem szerepelnek a MyBPHS-ben.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="max-h-96 overflow-y-auto">
                    {orphanedADUsers.length > 0 ? (
                      <div className="space-y-2">
                        {orphanedADUsers.map((adUser, index) => (
                          <div
                            key={adUser.UserPrincipalName || `ad-user-${index}`}
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-sm text-gray-400">
                                  Bejelentkezési név: {adUser.SamAccountName}
                                </p>
                              </div>
                      ...
                      <p className="text-center text-gray-300">
                        Nincsenek orphaned AD felhasználók.
                      </p>
                    )}
                  </div>
                </DialogContent>
              ...
              >
                <TrashIcon className="size-4" />
                Törlés {selectedRows.size > 0 && `(${selectedRows.size})`}
              </Button>
              ...
              {orphanedADUsers.length > 0 && (
                ...
                >
                  <DialogTrigger asChild>
                    ...
                    >
                      <TriangleAlertIcon className="size-4" />
                      AD figyelmeztetés ({orphanedADUsers.length})
                    </Button>
                  </DialogTrigger>
                </Dialog>
              )}
            ...
            <TableBody>
              {isLoading ? (
                ...
                paginatedUsers.map((user, index) => {
                  ...
                  const displayUser =
                    isEditing && editingUserData ? editingUserData : user;
                        ...
                        <TableCell key={column.key} className="text-white">
                          ...
                          {column.key === "name" && (
                            <div className="flex flex-col">
                              {isEditing && getFieldConfig("name")?.editable ? (
                                <Input
                                  ...
                                  <DropdownMenuContent className="border-gray-600 bg-[#242424]">
                                    {roleKeys.map((role) => (
                                      <DropdownMenuCheckboxItem
                                        ...
                                        onCheckedChange={() => {
                                          const newRoles =
                                            displayUser.roles.includes(role)
                                              ? displayUser.roles.filter(
                                                  (r) => r !== role,
                                                )
                                              : [...displayUser.roles, role];
                                          updateEditingField("roles", newRoles);
                                        }}
                                        className="font-medium text-white hover:bg-[#2e2e2e] hover:text-white focus:bg-[#2e2e2e] focus:text-white"
                                      >
                                        {roles[role] ?? role}
                            ...
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {!displayUser.hasADAccount && (
                                <Button
                                  variant="outline"
                                  size="icon"
                                  onClick={() => handleCreateADUser(user)}
        ...
        <div className="flex flex-col items-center justify-between gap-2 rounded-lg border border-gray-600 bg-[#2e2e2e] p-2 sm:gap-4 sm:p-4 lg:flex-row">
          <div className="flex flex-row items-center justify-between gap-10">
            <div className="flex items-center gap-2 text-xs text-gray-300 sm:gap-3 sm:text-sm">
              ...
              <span className="font-medium sm:hidden">Sorok/oldal:</span>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
            ...
            <div className="flex items-center gap-1 text-xs font-medium text-gray-300 sm:gap-2 sm:text-sm">
              <span className="hidden sm:inline">
                ...
                -{" "}
                <span className="text-white">
                  {Math.min(page * pageSize, filteredAndSortedUsers.length)}
                </span>{" "}
                /{" "}
                <span className="text-white">
                  {filteredAndSortedUsers.length}
